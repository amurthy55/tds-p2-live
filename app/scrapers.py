...
def _maybe_fetch_github_tree(page_html_or_json, collected_pages):
    """
    Detect GitHub tree specification and fetch the recursive tree listing.
    Adds it as another 'page' entry.
    Never used in LLM sandbox (safe).
    """
    try:
        # Try to parse JSON first
        data = json.loads(page_html_or_json)
    except Exception:
    ...
    logger.info(f"[GH-TREE] Fetching GitHub tree: {api_url}")
